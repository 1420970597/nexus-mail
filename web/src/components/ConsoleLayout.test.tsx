...
describe('ConsoleLayout', () => {
  it('renders canonical quick-action titles for shared routes and keeps the current page out of quick actions', () => {
    ...
    const pageSummary = screen.getByRole('region', { name: '当前页面摘要' })
    expect(within(pageSummary).getByRole('heading', { name: '风控中心' })).toBeInTheDocument()
    ...
    expect(screen.getByRole('contentinfo')).toHaveTextContent('单一登录后控制台 · 共享布局骨架 · 按角色差异化菜单与页面')
