...
import { IconActivity, IconArticle, IconArrowRight, IconBolt, IconSafe, IconServer } from '@douyinfe/semi-icons'
...
function surfaceItems(role?: string) {
  const baseItems = [
  ...
  if (role === 'admin') {
    return [...baseItems, { label: '风险 / 审计回放', route: '/admin/risk + /admin/audit', summary: '高危事件复盘' }]
  }

  if (role === 'supplier') {
    return [...baseItems, { label: '资源 / 供货工作台', route: '/supplier/resources', summary: '供给链路联调' }]
  }

  return [...baseItems, { label: '项目 / 订单回放', route: '/projects + /orders', summary: '真实业务验证' }]
}
...
export function ApiDocsPage() {
  ...
  const surfaces = useMemo(() => surfaceItems(user?.role), [user?.role])
