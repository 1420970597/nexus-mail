...
describe('LoginPage', () => {
  ...
  it('renders a denser shared-console entry with a compact signal strip and product-style registration runway', () => {
    ...
    expect(registerJourneyScope.getByRole('heading', { name: '文档核对' })).toBeInTheDocument()
    ...
    expect(registerJourneyScope.getByText('真实回调验证')).toBeInTheDocument()
    expect(registerJourneyScope.getByText('确认回调地址并发起一次真实联调。')).toBeInTheDocument()
    expect(registerJourneyScope.getByText('契约核对')).toBeInTheDocument()
    expect(registerJourneyScope.getByText('查看 API 文档并核对请求契约。')).toBeInTheDocument()
