...
describe('LoginPage', () => {
  ...
  it('renders a denser shared-console entry with a compact readiness summary and shortened registration journey', () => {
    ...
    expect(readinessScope.getByText('统一认证')).toBeInTheDocument()
    expect(readinessScope.getByText('登录 / 注册同入口')).toBeInTheDocument()
    expect(readinessScope.getByText('共享 shell')).toBeInTheDocument()
    expect(readinessScope.getByText('注册成功直达控制台')).toBeInTheDocument()
    expect(readinessScope.getByText('角色菜单')).toBeInTheDocument()
    expect(readinessScope.getByText('按服务端角色切换')).toBeInTheDocument()
