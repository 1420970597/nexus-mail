...
describe('LoginPage', () => {
  ...
  it('renders a denser shared-console entry with a compact signal strip and product-style registration runway', () => {
    ...
    expect(entrySummary.getByText('统一入口、共享壳与接入路径，同一登录面完成切换。')).toBeInTheDocument()
    expect(entrySummary.queryByText('统一入口、共享控制台、接入链路，在同一登录面完成切换。')).not.toBeInTheDocument()
    ...
    expect(registerJourneyScope.getByText('注册后进入共享控制台，按同一导航完成首轮接入。')).toBeInTheDocument()
    expect(registerJourneyScope.queryByText('注册后进入共享控制台，沿同一导航完成首轮接入闭环。')).not.toBeInTheDocument()
    ...
    expect(within(apiKeysRunway).getByText('生成首个最小权限密钥，作为接入起点。')).toBeInTheDocument()
    expect(within(apiKeysRunway).queryByText('生成首个最小权限密钥。')).not.toBeInTheDocument()
    ...
    expect(within(webhooksRunway).getByText('确认回调地址，随后发起真实联调。')).toBeInTheDocument()
    expect(within(webhooksRunway).queryByText('确认回调地址并发起真实联调。')).not.toBeInTheDocument()
