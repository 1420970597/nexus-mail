...
describe('LoginPage', () => {
  ...
  it('renders a denser shared-console entry with a compact signal strip and product-style registration runway', () => {
    ...
    expect(entrySummary.getByText('统一入口、共享壳与接入路径，在同一登录面完成切换。')).toBeInTheDocument()
    expect(entrySummary.queryByText('统一入口、共享壳与接入路径，同一登录面完成切换。')).not.toBeInTheDocument()
  ...
  it('switches the embedded auth guidance when moving from login to register mode', async () => {
    ...
    expect(registerBanner.getByText(/注册后直接进入共享控制台，沿同一导航前往项目市场、订单中心与 API Keys。/)).toBeInTheDocument()
    expect(within(screen.getByTestId('login-auth-copy-block')).getByText('注册后直接进入共享控制台，沿同一导航前往项目市场、订单中心与 API Keys。')).toBeInTheDocument()
