...
describe('SettingsPage', () => {
  ...
  it('exposes the shared-console bridge as a named region with canonical role-aware mission cards for regular users', async () => {
    ...
    expect(shortcutScope.getByRole('button', { name: '打开 API Keys' })).toBeInTheDocument()
    expect(shortcutScope.getByRole('button', { name: '继续配置 Webhook' })).toBeInTheDocument()
    expect(shortcutScope.queryByRole('button', { name: '管理 API Keys' })).not.toBeInTheDocument()
    expect(shortcutScope.queryByRole('button', { name: '打开 Webhook 设置' })).not.toBeInTheDocument()
