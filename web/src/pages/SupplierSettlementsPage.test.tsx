...
describe('SupplierSettlementsPage', () => {
  ...
  it('renders supplier finance mission-control shell with metrics and shared-console guidance', async () => {
    ...
    expect(missionScope.getByText('先核对待结算与冻结资金')).toBeInTheDocument()
    expect(missionScope.getByText('继续维护项目成本模型')).toBeInTheDocument()
    expect(missionScope.getByText('最后复盘争议与共享控制台入口')).toBeInTheDocument()
