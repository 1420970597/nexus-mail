...
describe('AdminUsersPage shared-console admin workbench', () => {
  ...
  it('renders admin finance mission-control shell with metrics and shared-console guidance', async () => {
    ...
    expect(screen.getByText('即使当前是管理员资金运营切片，也要保持单一登录后控制台叙事：完成账务 / 争议动作后，仍通过 API Keys、Webhook 与文档入口继续验证平台对外接入链路。')).toBeInTheDocument()
  ...
  it('suppresses unavailable shared-console CTAs and falls back to dashboard when only the finance page remains', async () => {
    ...
    const bridgeCard = screen.getByText('共享接入桥接').closest('.semi-card')
    expect(bridgeCard).not.toBeNull()
    expect(within(bridgeCard as HTMLElement).queryByText('API Keys · /api-keys')).not.toBeInTheDocument()
    expect(within(bridgeCard as HTMLElement).queryByText('Webhook 设置 · /webhooks')).not.toBeInTheDocument()
    expect(within(bridgeCard as HTMLElement).queryByText('API 文档 · /docs')).not.toBeInTheDocument()
