...
func (s *Service) ListAdminAudit(ctx context.Context, filter AdminAuditFilter) ([]APIKeyAuditEntry, error) {
	...
	if filter.Action != "" {
		...
		case "create", "update_whitelist", "update_risk_rule", "admin_wallet_adjustment", "update_supplier_cost_profile", "resolve_dispute", "revoke", "success", "denied_invalid", "denied_scope", "denied_whitelist", "denied_rate_limit":
		default:
