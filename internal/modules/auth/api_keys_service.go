...
func (s *Service) ListAdminAudit(ctx context.Context, filter AdminAuditFilter) ([]APIKeyAuditEntry, error) {
	...
	if filter.Action != "" {
		...
		case "create", "update_whitelist", "update_risk_rule", "update_sender_blacklist", "sender_blacklist_hit", "admin_wallet_adjustment", "update_supplier_cost_profile", "settle_supplier_pending", "resolve_dispute", "revoke", "success", "denied_invalid", "denied_scope", "denied_whitelist", "denied_rate_limit", "rate_limit_backend_error":
		default:
