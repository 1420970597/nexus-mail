...
### 最新执行检查点（2026-05-03）
- 本轮继续按“前端优先”收敛 `SupplierOfferingsPage` shared-console fallback 语义：将共享接入入口全部缺失时的回退动作从裸按钮升级为带说明文案的 scoped fallback 卡片，并新增专用 `supplier-offerings-shared-console-fallback-button` 作为唯一真实点击目标；同步把 `SupplierOfferingsPage.test.tsx` 从点击整块容器收敛为先断言 fallback 卡片说明文案，再点击专用按钮验证回到推荐工作台，继续保持 new-api 风格单一登录后深色共享控制台、角色菜单 gating 与 supplier 页 fallback / bridge 契约不变，而不是让说明容器本身承担可点击语义。
- 控制器已重新通过 `pnpm --dir web exec vitest run src/pages/SupplierOfferingsPage.test.tsx`、`pnpm --dir web build`、`go test ./...`、`docker compose up -d --build web api gateway`，并执行真实 API 回放脚本 `.hermes/local-output/run_supplier_offerings_shared_fallback_real_api.sh`（最新产物目录 `supplier-offerings-shared-fallback-20260503-181313`），确认 `GET /healthz`（200，`redis=ok`）、普通用户 `POST /api/v1/auth/register`（201）、`GET /api/v1/auth/menu`、`GET /api/v1/auth/me`、`GET /api/v1/dashboard/overview`、`GET /api/v1/auth/api-keys`、`GET /api/v1/webhooks/endpoints`、`GET /api/v1/projects/inventory`、`GET /api/v1/orders/activations`、`GET /api/v1/supplier/resources/overview`（预期 403）与前端 `/supplier/offerings`、`/api-keys`、`/webhooks`、`/docs` SPA 壳 200 均保持成立，本轮 fallback 语义收敛未破坏注册后共享菜单、真实接入接口、supplier 权限边界与单壳前端入口。
- 本轮五维评审结论：产品/规格 PASS、代码质量 APPROVED、安全/集成 SAFE、测试/可靠性 PASS、性能/运维 PASS；测试/可靠性评审仅记录一项非阻塞后续项：`SupplierOfferingsPage.test.tsx` 当前通过 `within(fallback).getAllByText('返回推荐工作台')` 断言标题与按钮双文案，后续如文案再演化可继续收敛为更细的 heading/button 组合断言，但当前专用按钮 test id 已真实覆盖导航合同且不构成阻塞。下一前端优先项转为继续清理其余 shared-console 页面中仍把 shared fallback 实现为裸按钮或依赖容器点击的场景，然后再回到 Phase 4/5 其余后端能力。
- 本轮继续按“前端优先”收敛 `SupplierResourcesPage` focused 回归中 shared-console fallback / mission fallback 的按钮归属歧义：新增 scoped 用例，显式验证 `supplier-resources-mission-fallback` 仅保留“返回推荐工作台继续供应商主链路”说明文案而不承载可点击按钮，真正可点击的 `返回推荐工作台` 只属于 `supplier-resources-shared-console-fallback` 区域，并继续保持 new-api 风格单一登录后深色共享控制台、角色菜单 gating 与真实回退导航合同不变。
- 控制器已重新通过 `pnpm --dir web exec vitest run src/pages/SupplierResourcesPage.test.tsx src/pages/SupplierOfferingsPage.test.tsx src/pages/SupplierSettlementsPage.test.tsx src/pages/AdminUsersPage.shared-console.test.tsx`、`pnpm --dir web build`、`go test ./...`、`docker compose up -d --build web api gateway`，并执行真实 API 回放脚本 `.hermes/local-output/run_supplier_resources_fallback_scope_real_api.sh`（产物目录 `supplier-resources-fallback-scope-20260503-174910`），确认 `GET /healthz`（200，`redis=ok`）、普通用户 `POST /api/v1/auth/register`、`GET /api/v1/auth/me`、`GET /api/v1/auth/menu`、`GET /api/v1/dashboard/overview`、`GET /api/v1/auth/api-keys`、`GET /api/v1/webhooks/endpoints`、`GET /api/v1/supplier/resources/overview`（预期 403）与前端 `/supplier/resources`、`/api-keys`、`/webhooks`、`/docs` SPA 壳 200 均保持成立，本轮 focused 去歧义化未破坏注册后共享菜单、共享接入接口、supplier 权限边界与单壳前端入口。
- 本轮五维评审结论：产品/规格 PASS、代码质量 APPROVED、安全/集成 SAFE、测试/可靠性 PASS、性能/运维 PASS；评审仅记录两项非阻塞后续项：如后续继续扩张此类 fallback 契约，可考虑把 shared fallback 按钮查询进一步绑定到 bridge/card 自身语义；继续保留既有 Semi UI `findDOMNode`、RTL `act(...)` 与前端 chunk 体积告警为非阻塞债务。下一前端优先项转为继续清理其余 shared-console focused 用例里残留的 fallback 按钮归属/重复文案歧义，然后再回到 Phase 4/5 其余后端能力。
- 本轮继续按“前端优先”收敛管理员 / 资金 / 供应商资源 shared-console focused 回归中的残余全局 CTA 假设：为 `AdminRiskPage` 与 `AdminAuditPage` 的“管理员主任务流”/“共享接入桥接”补充最小化语义锚点 `admin-risk-mission-flow`、`admin-risk-shared-console-bridge`、`admin-audit-mission-flow`、`admin-audit-shared-console-bridge`，并将 `AdminRiskPage.test.tsx`、`AdminAuditPage.test.tsx`、`AdminSuppliersPage.test.tsx`、`BalancePage.test.tsx`、`SupplierResourcesPage.test.tsx` 从页面级 CTA 缺失判断收敛为 mission-flow / bridge / fallback 作用域内断言，同时把管理员风控/审计导航测试改为显式 `unmount()` 以避免多棵 Router 树残留；继续保持 new-api 风格单一登录后深色共享控制台、角色菜单 gating 与真实导航合同不变，而不是继续依赖全页同名按钮缺失或跨区域误匹配。
- 控制器已重新通过 `pnpm --dir web exec vitest run src/pages/AdminRiskPage.test.tsx src/pages/AdminAuditPage.test.tsx src/pages/AdminSuppliersPage.test.tsx src/pages/BalancePage.test.tsx src/pages/SupplierResourcesPage.test.tsx`、`pnpm --dir web build`、`go test ./...`、`docker compose up -d --build web api gateway`，并执行真实 API 回放脚本 `.hermes/local-output/run_front_shared_console_tightening_real_api.sh`（产物目录 `front-shared-console-tightening/20260503-172521`），确认 `GET /healthz`（200，`redis=ok`）、普通用户 `POST /api/v1/auth/register`、`GET /api/v1/auth/me`、`GET /api/v1/auth/menu`、`GET /api/v1/dashboard/overview`、`GET /api/v1/auth/api-keys`、`GET /api/v1/webhooks/endpoints`、管理员 `POST /api/v1/auth/login`、`GET /api/v1/auth/menu`、`GET /api/v1/admin/overview`、`GET /api/v1/admin/risk`、`GET /api/v1/admin/audit?limit=5` 与前端 `/balance`、`/api-keys`、`/supplier/resources`、`/admin/risk`、`/admin/audit` SPA 壳 200 均保持成立，本轮 focused 去脆弱化未破坏注册后共享菜单、管理员真实运营接口与单壳前端入口。
- 本轮五维评审结论：产品/规格 PASS、代码质量 APPROVED、安全/集成 SAFE、测试/可靠性 PASS、性能/运维 PASS；评审仅记录三项非阻塞后续项：`SupplierResourcesPage.test.tsx` 未来可把 `cleanup()` 进一步统一为显式 `unmount()`；`AdminSuppliersPage.test.tsx` 的 bridge fallback 断言后续可继续下沉到 fallback 卡片自身；继续保留既有 Semi UI `findDOMNode`、RTL `act(...)` 与前端 chunk 体积告警为非阻塞债务。下一前端优先项转为继续清理其余 shared-console focused 用例里残留的全局“返回推荐工作台”缺失断言与 bridge / fallback 混合作用域问题，然后再回到 Phase 4/5 其余后端能力。
- 本轮继续按"前端优先"收敛 `BalancePage` focused 回归中的跨区域 CTA / capability matrix 选择器耦合：保持单一登录后 new-api 风格深色共享控制台、角色菜单 gating 与真实导航合同不变，仅将 `web/src/pages/BalancePage.test.tsx` 中 Finance Mission Control 的项目市场 / 订单中心 / API Keys / Webhook / 文档导航，从页面级文本点击收敛为 `balance-mission-cards`、`balance-orders-mission-card` 与 capability actions 语义锚点内的 scoped 查询；同时在 `BalancePage` 为集成任务按钮与 capability actions 补充最小化语义锚点 `balance-open-api-keys` / `balance-open-webhooks` / `balance-open-docs` 与 `balance-capability-actions`，避免未来同名 CTA 在其他卡片或指标区出现时误通过。
- 控制器已重新通过 `pnpm --dir web exec vitest run src/pages/BalancePage.test.tsx`、`pnpm --dir web build`、`go test ./...`，并执行真实 API 回放脚本 `.hermes/local-output/run_balance_shared_console_hardening_real_api.sh`（产物目录 `balance-shared-console-hardening-20260503-165438`），确认 `GET /healthz`（200，`redis=ok`）、普通用户 `POST /api/v1/auth/register`、`GET /api/v1/auth/menu`、`GET /api/v1/auth/me`、`GET /api/v1/dashboard/overview`、`GET /api/v1/wallet/overview`、`GET /api/v1/wallet/transactions`、`GET /api/v1/auth/api-keys`、`GET /api/v1/webhooks/endpoints` 与前端 `/balance`、`/api-keys`、`/webhooks`、`/docs` SPA 壳 200 均保持成立，本轮 focused 去脆弱化未破坏注册后共享菜单、真实资金接口与单壳前端入口。
- 本轮五维评审结论：产品/规格 PASS、代码质量 APPROVED、安全/集成 SAFE、测试/可靠性 PASS、性能/运维 PASS；首轮代码质量 / 安全集成复评指出 `BalancePage` 集成任务按钮误用 Semi UI `theme="tertiary"` 会引入新的 prop warning，控制器已立即恢复为受支持的 `theme="borderless"` + `type="tertiary"` 并复验通过。继续保留既有 Semi UI `findDOMNode`、RTL `act(...)` 与前端 chunk 体积告警为非阻塞债务；下一前端优先项转为继续清理其余 shared-console focused 用例里残留的 capability / bridge 跨区域 CTA 弱作用域查询，然后再回到 Phase 4/5 其余后端能力。
- 本轮继续按"前端优先"收敛 `ProfilePage` focused 回归中的共享 fallback / bridge 全局 CTA 查询耦合：保持单一登录后 new-api 风格深色共享控制台、角色菜单 gating 与真实导航合同不变，仅将 `web/src/pages/ProfilePage.test.tsx` 中用户 bridge 的 `API Keys / Webhook / Docs` 导航，以及 user / supplier / admin 三类 shared-console return CTA 断言与点击，从页面级 `screen.getByRole(...)` 收敛为 `profile-capability-bridge` 与 `profile-shared-console-return` 语义区域内的 `within(...)` scoped 查询，避免未来同名按钮/文案在其他卡片出现时误通过。
- 控制器已重新通过 `pnpm --dir web exec vitest run src/pages/ProfilePage.test.tsx`、`pnpm --dir web build`、`go test ./...`，并执行真实 API 回放脚本 `.hermes/local-output/run_profile_shared_console_real_api.sh`（产物目录 `profile-shared-console-fallback-20260503-162053`），确认 `GET /healthz`（200，`redis=ok`）、普通用户 `POST /api/v1/auth/register`、`GET /api/v1/auth/menu`、`GET /api/v1/auth/me`、`GET /api/v1/dashboard/overview`、`GET /api/v1/auth/api-keys`、`GET /api/v1/webhooks/endpoints` 与前端 `/profile`、`/api-keys`、`/webhooks`、`/settings` SPA 壳 200 均保持成立，本轮 focused 去脆弱化未破坏注册后共享菜单、真实接入接口与单壳前端入口。
- 本轮五维评审结论：产品/规格 PASS、代码质量 APPROVED、安全/集成 SAFE、测试/可靠性 PASS、性能/运维 PASS；测试/可靠性与性能/运维仅记录两项非阻塞后续项：未来如共享桥接/回退区域继续扩张，可再评估是否抽轻量 helper 或更多语义查询以减少 `data-testid` 依赖；继续保留既有 Semi UI `findDOMNode`、RTL `act(...)` 与前端 chunk 体积告警为非阻塞债务。下一前端优先项转为继续清理其余 shared-console focused 用例里残留的跨区域 CTA / fallback 全局查询耦合，然后再回到 Phase 4/5 其余后端能力。
- 本轮继续按"前端优先"收敛 `SupplierSettlementsPage` focused 回归里仍残留的重复 fallback 文案歧义：补充一条新的 scoped 测试，显式验证当 mission fallback 与 shared-console fallback 同时存在时，真正可点击的 `返回推荐工作台` 仅属于 `supplier-settlements-mission-fallback` 区域，而 shared-console fallback 卡片本身只保留说明文案，不再让后续 focused 用例误把同名文案当成可点击 CTA；继续保持 new-api 风格单一登录后深色共享控制台、角色菜单 gating 与真实回退导航合同不变，而不是继续依赖全局重复文案隐式区分两个 fallback。
- 控制器已重新通过 `pnpm --dir web exec vitest run src/pages/SupplierSettlementsPage.test.tsx`、`pnpm --dir web build`、`go test ./...`，并执行真实 API 回放脚本 `.hermes/local-output/run_supplier_settlements_fallback_scope_real_api.sh`（产物目录 `supplier-settlements-fallback-scope-20260503-153702`），确认 `GET /healthz`（200，`redis=ok`）、普通用户 `POST /api/v1/auth/register`、`GET /api/v1/auth/menu`、`GET /api/v1/auth/me`、`GET /api/v1/dashboard/overview`、`GET /api/v1/auth/api-keys`、`GET /api/v1/webhooks/endpoints` 与前端 `/supplier/settlements` SPA 壳 200 均保持成立，本轮 focused 去歧义化未破坏注册后共享菜单、真实接入接口与单壳前端入口。
- 本轮五维评审结论：产品/规格 PASS、代码质量 APPROVED、安全/集成 SAFE、测试/可靠性 PASS、性能/运维 PASS；安全/集成评审仅给出一项非阻塞建议：未来如需进一步补强，可再单独验证 `supplier-settlements-shared-console-fallback-button` 在混合 fallback 场景下的点击路径，但当前 diff 已精确锁定本轮真正待修的重复 fallback 文案歧义且不构成阻塞。继续保留既有 Semi UI `findDOMNode`、async-validator `currency is required` 控制台噪声与前端 chunk 体积告警为非阻塞债务；下一前端优先项转为继续清理其余 shared-console focused 用例里残留的“重复文案存在但仅一处可点击”歧义，再回到 Phase 4/5 其余后端能力。
- 本轮继续按“前端优先”收敛 `ApiKeysPage` / `SupplierDomainsPage` focused 回归中残留的表格 DOM 结构耦合：已将 `web/src/pages/ApiKeysPage.test.tsx` 与 `web/src/pages/SupplierDomainsPage.test.tsx` 的行级 helper 从 `cell.closest('tr')` / `[role="row"]` 双分支改为基于可访问性行名的 `getByRole('row', { name: ... })` 查询，并对动态名称做正则转义，继续保持 new-api 风格单一登录后深色共享控制台、角色菜单 gating 与真实 API / 域名运营合同不变，而不是继续依赖 Semi UI 表格输出结构。
- 控制器已重新通过 `pnpm --dir web exec vitest run src/pages/ApiKeysPage.test.tsx src/pages/SupplierDomainsPage.test.tsx`、`pnpm --dir web build`、`go test ./...`、`docker compose up -d --build web api gateway`，并执行真实 API 回放脚本 `.hermes/local-output/run_api_keys_supplier_domains_row_a11y_real_api.sh`（产物目录 `api-keys-supplier-domains-row-a11y-20260503-152014`），确认 `GET /healthz`（200，`redis=ok`）、普通用户 `POST /api/v1/auth/register`、`GET /api/v1/auth/menu`、`GET /api/v1/auth/me`、`GET /api/v1/dashboard/overview`、`GET /api/v1/auth/api-keys`、`GET /api/v1/webhooks/endpoints`、`GET /api/v1/projects/inventory`、`GET /api/v1/orders/activations` 与前端 `/api-keys`、`/supplier/domains` SPA 壳 200 均保持成立，本轮 focused 去脆弱化未破坏注册后共享菜单、真实接入接口与供应商域名工作台入口。
- 本轮五维评审结论：产品/规格 PASS、代码质量 APPROVED、安全/集成 SAFE、测试/可靠性 PASS、性能/运维 PASS；评审仅提示一项已当场吸收的非阻塞健壮性点：行名匹配 helper 需对动态名称做正则转义，控制器已补入转义后复验通过。继续保留既有 Semi UI `findDOMNode`、RTL `act(...)` 与前端 chunk 体积告警为非阻塞债务；下一前端优先项转为继续清理其余 shared-console focused 用例中仍残留的可访问性名称/表格作用域弱耦合，然后再回到 Phase 4/5 其余后端能力。
- 本轮继续按"前端优先"收敛 `ApiKeysPage` focused 回归中的低信号全局文本计数断言：已为“最近使用”指标卡与“审计日志”卡片补充最小化语义锚点 `api-keys-latest-usage-metric` / `api-keys-audit-log-card`，并将 `web/src/pages/ApiKeysPage.test.tsx` 中原本依赖 `getAllByText(...).length` 的宽泛存在性检查收敛为 `within(...)` 作用域断言，继续保持 new-api 风格单一登录后深色共享控制台、真实菜单 gating 与 API Keys → Webhook → Docs 接入链路不变，而不是继续依赖重复时间/数字文本在全页任意位置出现。
- 控制器已重新通过 `pnpm --dir web exec vitest run src/pages/ApiKeysPage.test.tsx`、`pnpm --dir web build`、`go test ./...`，并执行真实 API 回放脚本 `.hermes/local-output/run_api_keys_focused_hardening_real_api.sh`（产物目录 `api-keys-focused-hardening-20260503-140713`），确认 `GET /healthz`（200，`redis=ok`）、普通用户 `POST /api/v1/auth/register`、`GET /api/v1/auth/me`、`GET /api/v1/auth/menu`、`GET /api/v1/dashboard/overview`、`GET /api/v1/auth/api-keys`、`GET /api/v1/webhooks/endpoints` 与前端 `/api-keys`、`/webhooks`、`/settings` SPA 壳 200 均保持成立，本轮 focused 去脆弱化未破坏注册后共享菜单、真实接入接口与单壳前端入口。
- 本轮五维评审结论：产品/规格 PASS、代码质量 APPROVED、安全/集成 SAFE、测试/可靠性 PASS、性能/运维 PASS；代码质量与可靠性评审仅记录一项非阻塞后续项：未来如继续清理该页，可优先复用已有可见区域语义或更细粒度业务锚点，减少 test-only `data-testid` 扩散，但当前 diff 已显著优于先前的全局重复文本计数断言且不构成阻塞。继续保留既有 Semi UI `findDOMNode`、RTL `act(...)` 与前端 chunk 体积告警为后续非阻塞债务；下一前端优先项转为继续清理其余 shared-console focused 用例里残留的低信号全局计数 / 顺序索引耦合，然后再回到 Phase 4/5 其余后端能力。
- 本轮继续按"前端优先"清理 `AdminUsersPage.shared-console.test.tsx` 中残余的 `.closest('.semi-card')` mission-flow 结构耦合：已为 `AdminUsersPage` 的"管理员主任务流"卡片补充最小化 `data-testid="admin-users-mission-flow"` 语义锚点，并将风控 / 审计 / API Keys 三段导航断言统一收敛为 `within(mission-flow)` 作用域查询，继续保持 new-api 风格单一登录后深色共享控制台、角色菜单 gating 与真实导航合同不变，而不是继续依赖 Semi UI Card 包装层。
- 控制器已重新通过 `pnpm --dir web exec vitest run src/pages/AdminUsersPage.shared-console.test.tsx`、`pnpm --dir web build`、`go test ./...`、`docker compose up -d --build web api gateway`，并执行真实 API 回放脚本 `.hermes/local-output/run_admin_users_mission_flow_real_api.sh`（产物目录 `admin-users-mission-flow-20260503-145131`），确认 `GET /healthz`（200，`redis=ok`）、普通用户 `POST /api/v1/auth/register`、`GET /api/v1/auth/me`、`GET /api/v1/auth/menu`、`GET /api/v1/dashboard/overview`、`GET /api/v1/auth/api-keys`、`GET /api/v1/webhooks/endpoints`、管理员 `POST /api/v1/auth/login`、`GET /api/v1/auth/menu`、`GET /api/v1/admin/overview`、`GET /api/v1/admin/risk`、`GET /api/v1/admin/audit?limit=5` 与前端 `/admin/users`、`/api-keys`、`/settings` 壳 200 均保持成立，本轮 focused 去脆弱化未破坏注册后共享菜单、管理员真实运营接口与单壳前端入口。
- 本轮五维评审结论：产品/规格 PASS、代码质量 APPROVED、安全/集成 SAFE、测试/可靠性 PASS、性能/运维 PASS；性能/运维评审仅记录一项非阻塞后续项：若未来 UI 库升级不再稳定透传 `data-testid`，可再下沉到自有 wrapper 元素，但当前最小语义锚点方案已显著优于 `.closest('.semi-card')` 且不构成阻塞。继续保留既有 Semi UI `findDOMNode`、RTL `act(...)` 与前端 chunk 体积告警为后续非阻塞债务；下一前端优先项转为继续清理其余 shared-console focused 用例里残留的 `.closest(...)` / `parentElement` 结构耦合，然后再回到 Phase 4/5 其余后端能力。
- 本轮继续按“前端优先”清理 AdminUsers / Orders focused 回归中的残余全局导航查询耦合：已将 `AdminUsersPage.shared-console.test.tsx` 中管理员任务流跳转从页面级按钮查询收敛到 `管理员主任务流` 卡片作用域，并把 `OrdersPage.test.tsx` 中空态“查看 API 接入准备”导航从全局文本点击收敛为 `orders-empty-state-actions` 区域内的按钮查询；继续保持 new-api 风格单一登录后深色共享控制台、角色菜单 gating 与真实导航/真实 API 合同不变，而不是继续依赖同名 CTA 在全页唯一这一脆弱假设。
- 控制器已重新通过 `pnpm --dir web exec vitest run src/pages/AdminUsersPage.shared-console.test.tsx src/pages/OrdersPage.test.tsx`、`pnpm --dir web build`、`go test ./...`、`docker compose up -d --build web api gateway`，并执行真实 API 回放脚本 `.hermes/local-output/run_admin_users_orders_scoped_nav_real_api.sh`（产物目录 `admin-users-orders-scoped-nav-20260503-142908`），确认 `GET /healthz`（200，`redis=ok`）、普通用户 `POST /api/v1/auth/register`、`GET /api/v1/auth/me`、`GET /api/v1/auth/menu`、`GET /api/v1/dashboard/overview`、`GET /api/v1/auth/api-keys`、`GET /api/v1/webhooks/endpoints`、管理员 `POST /api/v1/auth/login`、`GET /api/v1/auth/menu`、`GET /api/v1/admin/overview`、`GET /api/v1/admin/risk`、`GET /api/v1/admin/audit?limit=5` 与前端 `/`、`/orders`、`/api-keys`、`/webhooks`、`/admin/users` 壳 200 均保持成立，本轮 focused 去脆弱化未破坏注册后共享菜单、管理员真实运营接口与单壳前端入口。
- 本轮五维评审结论：产品/规格 PASS、代码质量 APPROVED、安全/集成 SAFE、测试/可靠性 PASS、性能/运维 PASS；其中代码质量评审仅记录一项非阻塞后续项：`AdminUsersPage.shared-console.test.tsx` 仍通过 `.closest('.semi-card')` 锁定 `管理员主任务流` 卡片，后续可继续收敛为更稳定的语义锚点。继续保留既有 Semi UI `findDOMNode`、RTL `act(...)` 与前端 chunk 体积告警为非阻塞债务；下一前端优先项转为继续清理其余 shared-console focused 用例里残留的全局 CTA 文本 / 顺序索引耦合，然后再回到 Phase 4/5 其余后端能力。

- 本轮继续按“前端优先”收敛 `SupplierDomainsPage` focused 回归中的跨区域 CTA 全局查询耦合：保持单一登录后 new-api 风格深色共享控制台、角色菜单 gating 与真实路由合同不变，仅将“查看供应商资源 / 继续维护供货规则 / 打开 API Keys / 打开 供应商结算”导航断言从页面级 `getByRole(...)` 收紧为 `supplier-domains-mission-flow` 与 `supplier-domains-shared-console-bridge` 语义区域内的 `within(...)` scoped 查询，避免后续同名 CTA 出现在其他卡片时误通过。
- 控制器已重新通过 `pnpm --dir web exec vitest run src/pages/SupplierDomainsPage.test.tsx`、`pnpm --dir web build`、`go test ./...`、`docker compose up -d --build web api gateway`，并执行真实 API 回放脚本 `.hermes/local-output/run_supplier_domains_scoped_navigation_real_api.sh`（产物目录 `supplier-domains-scoped-navigation-20260503-125738`），确认 `GET /healthz`（200，`redis=ok`）、普通用户 `POST /api/v1/auth/register`、`GET /api/v1/auth/menu`、`GET /api/v1/auth/me`、`GET /api/v1/dashboard/overview`、`GET /api/v1/projects/inventory`、`GET /api/v1/orders/activations`、`GET /api/v1/auth/api-keys`、`GET /api/v1/webhooks/endpoints`、`GET /api/v1/supplier/resources/overview`（预期 403）与前端 `/supplier/domains` SPA 壳 200 均保持成立，本轮 focused 去脆弱化未破坏注册后共享菜单、真实接入接口与供应商资源权限边界。
- 本轮五维评审结论：产品/规格 PASS、代码质量 APPROVED、安全/集成 SAFE、测试/可靠性 PASS、性能/运维 PASS；代码质量复评仅指出表格行 helper 保留 `tr` / `[role="row"]` 双路径更稳妥，控制器已在最终 live diff 中恢复该兼容分支后复验通过。继续保留既有 Semi UI `findDOMNode`、RTL `act(...)` 与前端 chunk 体积告警为非阻塞债务；下一前端优先项转为继续清理其余 shared-console focused 用例里残留的跨区域 CTA 全局查询 / 语义区域未显式作用域化问题，然后再回到 Phase 4/5 其余后端能力。
- 本轮继续按"前端优先"收敛 `SupplierDomainsPage` focused 回归中的残余表格结构耦合：已抽取 `getDomainTableRow(...)` helper，统一从 `supplier-domains-table-card` 语义锚点内定位域名行，并将 Catch-All 状态断言从内联 `closest('[role="row"]')` 改为 helper + `within(row)` 作用域校验；同时把新增域名提交用例从脆弱的调用次数断言收敛为更贴近用户可见结果的成功 Toast 文案与新域名渲染断言，继续保持 new-api 风格单一登录后深色共享控制台、角色菜单 gating 与真实域名运营合同不变，而不是继续依赖 Semi UI 表格 DOM 形态或宽泛 reload 次数。
- 控制器已重新通过 `pnpm --dir web exec vitest run src/pages/SupplierDomainsPage.test.tsx src/pages/ApiKeysPage.test.tsx`、`pnpm --dir web build`、`go test ./...`、`docker compose up -d --build web api gateway`，并执行真实 API 回放脚本 `.hermes/local-output/run_supplier_domains_row_hardening_real_api.sh`（产物目录 `supplier-domains-row-hardening-20260503-123730`），确认 `GET /healthz`、普通用户 `POST /api/v1/auth/register`、`GET /api/v1/auth/menu`、`GET /api/v1/auth/me`、`GET /api/v1/dashboard/overview`、`GET /api/v1/auth/api-keys`、`GET /api/v1/webhooks/endpoints`、`GET /api/v1/projects/inventory`、`GET /api/v1/orders/activations` 与前端 `/supplier/domains` SPA 壳 200 均保持成立，本轮 focused 去脆弱化未破坏注册后共享菜单、真实接入接口与单壳前端入口。
- 本轮五维评审结论：产品/规格 PASS、代码质量 APPROVED、安全/集成 SAFE、测试/可靠性 PASS、性能/运维 PASS；其中代码质量评审仅建议后续继续把类似表格 helper 统一收口，测试/可靠性评审仅记录可在未来补更多行定位边界用例，当前 diff 不构成阻塞。继续保留既有 Semi UI `findDOMNode`、RTL `act(...)` 与前端 chunk 体积告警为后续非阻塞债务；下一前端优先项转为继续清理 `ApiKeysPage` 等 shared-console 页面里残留的表格行定位 / 重复 CTA 结构耦合，然后再回到 Phase 4/5 其余后端能力。
- 本轮继续按"前端优先"收敛 `App.test.tsx` 注册后 onboarding focused 回归的启动竞态与文案漂移：抽取 `openRegisterMode` / `submitRegistration` / `expectDefaultUserFirstRunLane` helper，先等待注册 CTA 真正可点击，再把断言从易漂移的欢迎文案收敛为 `dashboard-next-steps-lane` 内当前真实 next-steps 结构（`预算 → 采购 → 履约 → 接入`、`再进入项目市场采购`、`随后追踪订单履约`、`最后完成 API 接入` 与 `管理 API Keys`），继续保持 new-api 风格单一登录后深色共享控制台、注册后共享接入路径与角色菜单 gating 合同不变，而不是继续依赖 route-loading 过渡态或旧 onboarding 文案。
- 控制器已重新通过 `pnpm --dir web exec vitest run src/App.test.tsx`、`pnpm --dir web build`、`go test ./...`、`docker compose up -d --build web api gateway`，并执行真实 API 回放脚本 `.hermes/local-output/run_app_register_onboarding_real_api.sh`（产物目录 `app-register-onboarding-20260503-121053`），确认 `GET /healthz`、普通用户 `POST /api/v1/auth/register`、`GET /api/v1/auth/menu`、`GET /api/v1/auth/me`、`GET /api/v1/dashboard/overview`、`GET /api/v1/auth/api-keys`、`GET /api/v1/webhooks/endpoints` 与前端 `/api-keys` SPA 壳 200 均保持成立，注册后共享菜单继续暴露 `/api-keys`、`/webhooks`、`/settings` 等基础接入路径。
- 本轮五维评审结论：产品/规格 PASS、代码质量 APPROVED、安全/集成 SAFE、测试/可靠性 PASS、性能/运维 PASS；其中代码质量复评确认此前 blocker 来自中间 diff 失效，最终 live diff 仅为测试 helper 抽取与运行时文案对齐，不引入新的权限、运行时或部署风险。继续保留既有 Semi UI `findDOMNode`、RTL `act(...)` 与前端 chunk 体积告警为非阻塞债务；下一前端优先项转为继续清理其余 shared-console focused 用例中残留的启动竞态 / 过渡态依赖，再回到 Phase 4/5 其余后端能力。
- 本轮继续按“前端优先”收敛 `ApiKeysPage` shared-console fallback 合同与行级语义测试：将 API Keys 页在 `Projects / Webhooks / Docs` 均未由服务端 menu 暴露时的裸 `返回推荐工作台` 按钮升级为轻量化共享回退区块 `api-keys-shared-console-fallback`，保留单一登录后 new-api 风格深色共享控制台与真实菜单 gating 不变；同时把 `ApiKeysPage.test.tsx` 的回退导航收敛为 fallback 区块作用域点击，并将 API Key 表格行定位 helper 改为优先 `tr`、回退 `[role="row"]`，减少 Semi UI 表格 DOM 形态变化带来的 focused 脆弱性。
- 控制器已重新通过 `pnpm --dir web exec vitest run src/pages/ApiKeysPage.test.tsx`、`pnpm --dir web build`、`go test ./...`，并执行真实 API 回放脚本 `.hermes/local-output/run_api_keys_fallback_real_api.sh`（产物目录 `api-keys-fallback-20260503-114842`），确认普通用户 `POST /api/v1/auth/register`、`GET /api/v1/auth/menu`、`GET /api/v1/auth/me`、`GET /api/v1/dashboard/overview`、`GET /api/v1/auth/api-keys`、`GET /api/v1/webhooks/endpoints`、`GET /healthz` 与前端 `/api-keys` 壳 200 仍保持成立，注册后共享菜单继续暴露 `/api-keys`、`/webhooks`、`/docs` 基础接入路径。
- 本轮五维评审结论：产品/规格 PASS、代码质量 APPROVED、安全/集成 SAFE、测试/可靠性 PASS、性能/运维 PASS；代码质量首轮仅指出新增回退容器不应过度扩张为重型运行时 Card，控制器已收敛为轻量 `Space` 区块后复验通过。继续保留既有 Semi UI `findDOMNode`、RTL `act(...)` 与前端 chunk 体积告警为非阻塞债务；下一前端优先项转为继续清理其余 shared-console focused 用例里残留的表格行/回退区块弱语义耦合，然后再回到 Phase 4/5 其余后端能力。
