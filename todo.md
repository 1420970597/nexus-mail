...
- Phase 4：进行中（已完成用户钱包、冻结余额、成功扣费、超时退款、供应商待结算余额、管理员调账能力与基础结算页面；本轮新增管理员供应商待结算确认：`POST /api/v1/admin/supplier-settlements` 可将供应商 pending 结算流水原子标记为 settled，并将待结算余额迁移到已结算余额，写入 `settle_supplier_pending` 审计且纳入管理员审计过滤与 OpenAPI/前端入口；本轮补强供应商报表筛选：`GET /api/v1/supplier/reports` 支持 `from/to/limit`，服务端校验日期与上限、前端显式查询、OpenAPI 文档与真实 API/索引验证已完成；本轮进一步补强 supplier reports limit 契约：HTTP 层显式拒绝非整数 `limit`，服务层拒绝负数、保留 `0/空 => 默认 100` 与 `>200 => 截断 200`，补齐空白字符串默认值与错误文案断言测试，并通过真实 API 验证 `limit=abc/-1` 返回 400、`limit=%20%20/201` 返回 200；本轮交付供应商供货规则维护：新增供应商自有 `GET/POST /api/v1/supplier/resources/offerings`、供货规则页面与侧边栏入口，支持按项目/域名池维护售价、成功率、优先级和来源类型，列表按当前供应商隔离，真实 API 验证 400/200/403 与库存反映已完成；本轮补强管理员争议单处理体验：争议单列表支持显式 `status/limit` 查询草稿，避免输入变化触发请求风暴，并通过真实 API 验证 open/resolved 过滤、limit 与非法状态 400；本轮前端共享控制台升级进入第二个里程碑：在保持单一登录后控制台与角色扩展菜单模式下，用真实页面替换 `/supplier/domains` 与 `/admin/suppliers` 高优先级占位页，新增供应商域名运营页与管理员供应商运营页，强化 `DashboardPage` 管理员经营摘要与 `SettingsPage` 信息架构，并补齐对应页面定向测试；已通过前端定向 Vitest（含 `SupplierDomainsPage` / `AdminSuppliersPage` / `App` 路由覆盖）、`pnpm --dir web build`、Docker 重建后真实 API 验证管理员 `/auth/menu` + `/admin/overview`、供应商 `/auth/menu` + `/supplier/resources/overview` 与 Web 首页可访问；本轮继续完成共享控制台视觉与导航收敛：`ConsoleLayout` / `AppSidebar` 切为更贴近 new-api 的深色单壳工作台，按基础工作台 / 供应商扩展 / 管理员扩展分组菜单，顶栏增加角色说明与快捷跳转；`DashboardPage` 改为任务导向入口卡片并保留管理员深度统计与供应商待结算排行，`SettingsPage` 收敛为真实快捷入口控制面板；已通过 `pnpm --dir web build` 与 `pnpm --dir web test -- src/App.test.tsx` 验证前端变更；本轮继续优先补全前端集成工作台：修复 `WebhooksPage` 构建错误并补齐注册后共享控制台中的 Webhooks / 风控 / 审计真实页面收敛，统一为更贴近 new-api 的深色工作台指标卡、角色说明与操作流；其中 Webhooks 页改为聚合当前 endpoint 列表的 delivery 数据后再计算端点总数 / 投递成功 / 失败或排队中 / 最近回调与逐行健康度，避免局部数据伪装全局运维态，健康度明确区分 `稳定 / 排队中 / 异常 / disabled`；`AdminRiskPage` 新增风险指挥台头图、规则命中概览、处置建议与共享控制台指标卡，`AdminAuditPage` 新增审计回放与追踪头图、风险动作统计与高亮；补齐 `WebhooksPage.test.tsx` 与 `App.test.tsx` 覆盖新增聚合与页面元素，并通过 `pnpm --dir web test -- src/pages/WebhooksPage.test.tsx`、`pnpm --dir web test -- src/App.test.tsx`、`pnpm --dir web build` 以及真实 API 回放验证 `/healthz`、管理员 `/auth/menu`、`/admin/risk`、`/；本轮继续完成登录/注册与角色导引收敛：`LoginPage` 新增“注册后进入共享控制台”的显式引导卡、三类角色路径说明与“以服务端菜单/角色为准”的能力说明，保持注册契约仍仅 `email/password`；`DashboardPage` 新增角色工作台导引时间线与共享壳角色菜单映射卡，按 user/supplier/admin 展示真实下一步工作流与可执行跳转；补齐 `App.test.tsx` 覆盖注册 CTA 切换、供应商工作台导引与导航行为，并通过 `pnpm --dir web test -- src/App.test.tsx`、`pnpm --dir web build`、`go test ./...` 及真实 API 回放验证 `/healthz`、`POST /auth/register`、`GET /auth/menu`、`GET /dashboard/overview`、`POST /auth/logout`。admin/audit`、`/webhooks/endpoints`；当前下一步重点：继续把 API Keys / Webhooks / 风控 / 审计之间补上更强的跨页跳转与联动测试，然后再回到 Phase 4/5 的后端 Webhook/API Key/IP 白名单/限流/风控/审计深化）
