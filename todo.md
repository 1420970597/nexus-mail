...
- 本轮继续按“前端优先”收敛 supplier shared-console fallback focused 回归中的重复 fallback 文案计数脆弱点：将 `SupplierResourcesPage.test.tsx` 与 `SupplierOff

... [OUTPUT TRUNCATED - 54303 chars omitted out of 104303 total] ...

in(row)` 执行行内按钮查询，继续保持 new-api 风格单一登录后深色共享控制台、共享接入回退路径与真实 API Key / Webhook / Docs 菜单 gating 合同不变，而不是继续依赖按钮数组顺序。
