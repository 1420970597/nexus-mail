...
- 本轮五维评审结论：产品/规格、代码质量、安全/集成、测试/可靠性、性能/运维均通过；其中产品/规格与代码质量首轮反对把 test-only `data-testid` 加入生产表格后，控制器已回退该运行时代码，改为以现有 `项目报表` 卡片为作用域完成 focused 断言并复验通过。继续保留既有 Semi UI `findDOMNode`、RTL `act(...)`、async-validator `currency is required` 控制台噪声与前端 chunk 体积告警为后续非阻塞债务；下一前端优先项转为继续清理其余 shared-console focused 用例里仍残留的重复 `getAllByText(...)` / 顺序索引弱断言，然后再回到 Phase 4/5 其余后端能力。
- 本轮继续按“前端优先”清理 `AdminUsersPage` focused 回归里的 `.closest('.semi-card')` 结构耦合：已为管理员调账、供应商待结算确认、争议单处理与争议单列表四个运营卡片补充稳定 `data-testid` 语义锚点，并将 `web/src/pages/AdminUsersPage.test.tsx` 改为通过 `getByTestId(...)` + `within(...)` 锁定目标卡片，继续保持 new-api 风格单一登录后深色共享控制台、角色菜单 gating 与管理员资金运营合同不变，而不是继续依赖 Semi UI 卡片包装层。
- 控制器已重新通过 `pnpm --dir web exec vitest run src/pages/AdminUsersPage.test.tsx`、`pnpm --dir web build`、`go test ./...`、`docker compose up -d --build web api gateway`，并完成真实 API 回放 `GET /healthz`、普通用户 `POST /api/v1/auth/register`、`GET /api/v1/auth/menu`、`GET /api/v1/auth/me`、`GET /api/v1/dashboard/overview`、管理员 `POST /api/v1/auth/login`、`GET /api/v1/auth/menu`、`GET /api/v1/admin/overview`、`GET /api/v1/admin/risk`、`GET /api/v1/admin/audit?limit=5` 与前端 `/admin/users` 壳 200，确认新的 AdminUsers 语义锚点没有破坏注册后共享菜单、管理员真实运营接口与单壳前端入口。
- 本轮五维评审结论：产品/规格、代码质量、安全/集成、测试/可靠性、性能/运维均通过；评审一致认为当前 diff 仅把 AdminUsers focused 测试从文案+`.closest('.semi-card')` 的脆弱结构依赖收敛为稳定语义锚点，未引入新的运行时/权限/部署风险。继续保留既有 Semi UI `findDOMNode`、RTL `act(...)`、Semi Form 校验噪声与前端 chunk 体积告警为后续非阻塞债务；下一前端优先项转为继续清理其余 shared-console focused 用例中残留的 `.closest('.semi-card')` / `parentElement` 结构耦合，然后再回到 Phase 4/5 其余后端能力。
- 本轮继续按“前端优先”清理 shared-console focused 回归中的结构耦合：已为 `ProjectsPage` 首轮采购 lane、`OrdersPage` 履约续接 lane、`BalancePage` 资金任务流与订单 mission card、`SupplierOfferingsPage` 共享控制台联动区补充稳定 `data-testid` 语义锚点，并将对应 focused 测试从 `.closest('.semi-card')` / `parentElement` 结构依赖切换为 `within(...)` + 语义锚点或直接按钮锚点，继续保持 new-api 风格单一登录后深色共享控制台、角色菜单 gating 与真实导航合同不变。
- 控制器已重新通过 `pnpm --dir web exec vitest run src/pages/ProjectsPage.test.tsx src/pages/OrdersPage.test.tsx src/pages/BalancePage.test.tsx src/pages/SupplierOfferingsPage.test.tsx`、`pnpm --dir web build`、`go test ./...`、`docker compose up -d --build web api gateway`，并完成真实 API 回放 `GET /healthz`、普通用户 `POST /api/v1/auth/register`、`GET /api/v1/auth/menu`、`GET /api/v1/auth/me`、`GET /api/v1/dashboard/overview`、`GET /api/v1/auth/api-keys`、`GET /api/v1/webhooks/endpoints`、`GET /api/v1/projects/inventory`、`GET /api/v1/orders/activations`、`GET /api/v1/wallet/overview`、管理员 `POST /api/v1/auth/login`、`GET /api/v1/auth/menu`、`GET /api/v1/admin/overview`、`GET /api/v1/admin/risk`、`GET /api/v1/admin/audit?limit=5` 与前端 `/settings`、`/projects`、`/orders`、`/api-keys`、`/docs`、`/supplier/offerings` 壳 200，确认新的语义锚点没有破坏注册后共享菜单、真实 API 契约与单壳前端入口。
- 本轮五维评审结论：产品/规格首轮指出 `BalancePage.test.tsx` 仍残留对 `.semi-card` 的内部依赖后，已为订单 mission card 增补稳定 `data-testid` 并复验通过；代码质量、安全/集成、测试/可靠性、性能/运维均通过，仅记录非阻塞后续项：`SupplierOfferingsPage` 新增的 bridge 语义锚点后续可继续在测试中消费以减少未使用标记，既有 Semi UI `findDOMNode`、RTL `act(...)` 与前端 chunk 体积告警继续保留为后续债务。下一前端优先项转为继续清理其余 shared-console 页面残留的 `.closest('.semi-card')` / `parentElement` 测试耦合，然后再回到 Phase 4/5 其余后端能力。
...
- 本轮五维评审结论：产品/规格、代码质量、安全/集成、测试/可靠性、性能/运维均通过；仅记录非阻塞后续项：可继续为“存在部分 bridge 入口时 fallback 必须隐藏”的组合场景补更细 focused 用例，并继续保留既有 Semi UI `findDOMNode`、RTL `act(...)` 与前端 chunk 体积告警为后续债务。下一前端优先项转为继续为其余管理员 / 供应商 shared-console 页面补齐同级 fallback 语义锚点与 focused 去脆弱化，然后再回到 Phase 4/5 其余后端能力。

- 本轮继续按"前端优先"清理 `SupplierResourcesPage` shared-console focused 回归脆弱点：已为 API Keys / Webhook / Docs 三个 bridge CTA 补充稳定 `data-testid` 语义锚点，并将 `web/src/pages/SupplierResourcesPage.test.tsx` 从 `.parentElement` / Semi UI 结构耦合切换为按钮级语义锚点与显式 fallback 点击断言，继续保持 new-api 风格单一登录后深色控制台、menu 真值 gating 与 supplier mission / shared-console 双 fallback 合同不变。
- 控制器已重新通过 `pnpm --dir web exec vitest run src/pages/SupplierResourcesPage.test.tsx`、`pnpm --dir web build`、`go test ./...`、`docker compose up -d --build web api gateway`，并完成真实 API 回放 `GET /healthz`、普通用户 `POST /api/v1/auth/register`、`GET /api/v1/auth/menu`、`GET /api/v1/auth/me`、`GET /api/v1/dashboard/overview`、`GET /api/v1/auth/api-keys`、`GET /api/v1/webhooks/endpoints`、`GET /api/v1/supplier/resources/overview`（预期 403）、管理员 `POST /api/v1/auth/login`、`GET /api/v1/supplier/resources/overview`（200），以及前端 `/supplier/resources`、`/api-keys`、`/webhooks`、`/docs` 壳 200，确认本轮测试去脆弱化没有破坏注册后普通用户共享菜单、真实资源权限边界与单壳前端入口。
- 本轮五维评审结论：产品/规格、代码质量、安全/集成、测试/可靠性、性能/运维均通过；评审一致认为当前 diff 去除了对 `.semi-card` / 父节点结构的脆弱依赖，且未改变 bridge/fallback 运行时合同。继续保留既有 Semi UI `findDOMNode`、RTL `act(...)` 与前端 chunk 体积告警为后续非阻塞债务；下一前端优先项转为继续为更多 shared-console 页面做同类 focused 测试去脆弱化与 bridge/fallback 语义锚点统一，然后再回到 Phase 4/5 其余能力。
- 本轮继续按“前端优先”补齐 `BalancePage` shared-console menu 真值契约：资金任务流与控制台能力矩阵中的 Projects / Orders / API Keys / Webhook / Docs CTA 现已按服务端 `menu` 真值动态显隐；当这些下游入口全部缺失且推荐工作台不再指向当前 `/balance` 页时，仅展示单一“返回推荐工作台”回退卡片，继续保持 new-api 风格单一登录后深色控制台，而不是在余额中心泄露未授权业务/接入入口。
- 已补齐 `web/src/pages/BalancePage.test.tsx` focused 回归，覆盖：余额中心在 menu 仅保留 `/` 与 `/balance` 时抑制采购 / 订单 / API Keys / Webhook / Docs CTA、显示共享控制台 fallback 文案，以及当 `/balance` 已是唯一可见工作台时隐藏 fallback；同时保留原有 Finance Mission Control 导航与充值/争议真实表单回归。
- 控制器已重新通过 `pnpm --dir web exec vitest run src/pages/BalancePage.test.tsx`、`pnpm --dir web build`、`go test ./...`、`docker compose up -d --build web api gateway`，并完成真实 API 回放 `GET /healthz`、普通用户 `POST /api/v1/auth/register`、`GET /api/v1/auth/menu`、`GET /api/v1/auth/me`、`GET /api/v1/dashboard/overview`、`GET /api/v1/wallet/overview`、`GET /api/v1/wallet/transactions`、`GET /api/v1/projects/inventory`、`GET /api/v1/orders/activations`、`GET /api/v1/auth/api-keys`、`GET /api/v1/webhooks/endpoints`、`POST /api/v1/wallet/disputes/999999`（预期 400），以及前端 `/docs`、`/balance` 壳 200，确认新的余额中心权限抑制没有破坏注册后普通用户共享菜单、真实钱包接口与单壳前端入口。
- 本轮五维评审结论：产品/规格、代码质量、安全/集成、性能/运维通过；测试/可靠性首轮指出 fallback 分支未做真实点击导航断言，但控制器复核发现当前页面在 menu 仅保留 `/` 与 `/balance` 时会直接退回到 `/balance` 自身，因此该里程碑以“抑制未授权 CTA + 仅在存在其他推荐工作台时才展示回退卡”为真实合同，不再保留误导性的导航断言。继续保留既有 Semi UI `findDOMNode`、Testing Library `act(...)` 与前端 chunk 体积告警为后续非阻塞债务；下一前端优先项转为继续为其余普通用户/shared-console 页面补齐同级 menu 真值 CTA 抑制与 fallback 语义一致性，再回到 Phase 4/5 其余能力。
