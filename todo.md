...
3. webhook 配置与重试机制（已完成可观测性里程碑：`GET/POST /api/v1/webhooks/endpoints`、`POST /api/v1/webhooks/endpoints/:id/test-delivery` 与 `GET /api/v1/webhooks/endpoints/:id/deliveries`；创建时生成签名 secret 且仅首次返回，列表仅返回 `secret_preview`；已建立 `webhook_endpoints` / `webhook_deliveries` schema，test-delivery 写入 `pending` 重试状态记录但不执行远程网络调用；投递记录查询仅返回当前用户自有 endpoint 下最近重试状态；已加入 http/https 与本地/内网/link-local/internal host SSRF 拦截）
